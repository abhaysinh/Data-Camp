...
100XP
    ...
    1   Compute an ECDF from the actual time between no-hitters (nohitter_times). Use the ecdf() function you
        wrote in the prequel course.

    2   Create a CDF from the theoretical samples you took in the last exercise (inter_nohitter_time).

    3   Plot x_theor and y_theor as a line using plt.plot(). Then overlay the ECDF of the real data x and y
