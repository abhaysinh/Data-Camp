...
You can still use the .predict() method on the X that was used to fit the model, but it is not a good indicator
...
this new data point, as well as on the training data X that the model has already seen. Using .predict()
on X_new will generate 1 prediction, while using it on X will generate 435 predictions: 1 for each sample.
